...
- 오래 현장에서 일해 온 아동 재활치료사 (치료실에서 직접 아이·부모를 만나온 사람)
...
경력·전문성 표현 규칙 (반드시 준수):
- **"임상", "임상에서", "임상가"는 영상 전체에서 절대 사용 금지** (현장 치료사는 입으로 이 단어를 쓰지 않음).
- **"25년"이라는 숫자를 매 영상에서 강조하지 말 것.** 한 영상 안에서 1회 이하로만 자연스럽게 등장.
  매 도입부를 "25년 임상에서 ...", "25년 동안 지켜본 ..." 식으로 시작하는 것 금지.
- 경력을 드러낼 때는 매번 다른 표현으로 (한 영상 안에서 중복 사용 금지):
  "그동안 치료실에서 봐온 아이들 보면요" / "제가 만나온 아이들 중에" / "현장에서 부모님들 얘기를 들어보면"
  / "오래 이 일을 하다 보니" / "수많은 아이들을 옆에서 지켜봐온 입장에서" / "치료실에서 매일 만나는 풍경이에요"
  / "그동안 제가 겪어온 경우들 보면" / "이 일을 하면서 가장 자주 마주치는 게"`;
...
function referenceBlock(pdfText?: string): string {
  ...
  return [
    '',
    '참고 자료 (책에서 주제 관련 부분 발췌):',
    '아래는 사용자가 제공한 책 본문에서 이번 영상의 주제와 직접 관련된 부분만 추려진 발췌입니다.',
    '이 발췌의 어휘·실제 사례·비유·강조 포인트를 살려 짱샘 화법으로 변환하세요.',
    '발췌에 없는 내용(특히 일반 의학·교육 상식)을 임의로 만들어 더하지 마세요.',
    '발췌가 빈약하면 무리해서 사용하지 말고, 그 슬라이드 분량을 짧게 끝내거나 리서치·일반 화법으로 풀어가세요.',
    '---',
    pdfText.trim(),
    '---',
  ].join('\n');
...
export function buildPrompt(input: PromptInput): { system: string; user: string } {
  ...
  if (mode === 'dialogue') {
    ...
    const user = [
      ...
      hasGrounding
        ? '주제를 1차 축으로 잡고, 리서치·책 발췌에 등장하는 어휘·실제 사례·비유·근거를 살려서 작성하세요. 자료에 없는 사실(통계·연구·일반 의학 상식)을 임의로 만들어 더하지 마세요.'
        : '',
